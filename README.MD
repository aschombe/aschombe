## Hi, I'm Andrew! 👋

### About Me
```c
struct Student {
    char *name;
    char *education;
    # maybe a current role or position
    char *current_role;
    char *experience[1024];
    char *interests[1024];
};

struct Student aschombe = {
    .name = "Andrew Schomber",
    .education = "B.S. in Computer Science from Stevens Institute of Technology",
    .current_role = "Associate Cyber Applied Research Scientist at Lockheed Martin's ATL",
    .experience = {
        "IT Infrastructure Intern at ConnectiveRX",
        "ROC Engineering Intern at Dega Systems",
        "Course Assistant for Computer Architecture and Systems Programming"
    },
    .interests = {
        "Cybersecurity",
        "Systems and Embedded Programming",
        "Computer Architecture",
        "Operating Systems",
        "Information Technology and Automation",
    }
};
```

### Tech Stack
- Operating Systems:
    [![Linux](https://img.shields.io/badge/-Linux-333333?style=flat&logo=linux)](https://www.linux.org/)
    [![Windows](https://custom-icon-badges.demolab.com/badge/Windows-333333?logo=windows&logoColor=white)](https://www.microsoft.com/en-us/windows)
    [![MacOS](https://img.shields.io/badge/-MacOS-333333?style=flat&logo=apple)](https://www.apple.com/macos)

- Languages:
    [![C](https://img.shields.io/badge/-C-333333?style=flat&logo=c)](https://www.gnu.org/software/gnu-c-manual/)
    [![ARM Assembly](https://img.shields.io/badge/-ARM_Assembly-333333?style=flat&logo=ARM)](https://developer.arm.com/documentation/107829/0200/Assembly-language-basics)
    [![C++](https://img.shields.io/badge/-C++-333333?logo=cplusplus&logoColor=blue)](https://isocpp.org/)
    [![Python](https://img.shields.io/badge/-Python-333333?style=flat&logo=python)](https://www.python.org)
    [![Bash](https://img.shields.io/badge/-Bash-333333?style=flat&logo=gnu-bash)](https://www.gnu.org/software/bash/)
    
- Tools and Platforms:
    [![Neovim](https://img.shields.io/badge/-Neovim-333333?style=flat&logo=neovim)](https://neovim.io/)
    [![Git](https://img.shields.io/badge/-Git-333333?style=flat&logo=git)](https://git-scm.com/)
    [![GitHub](https://img.shields.io/badge/-GitHub-333333?style=flat&logo=github)](https://github.com/)
    [![Visual Studio Code](https://custom-icon-badges.demolab.com/badge/VSCode-333333.svg?logo=vsc)](https://code.visualstudio.com/)
    [![Logisim Evolution](https://img.shields.io/badge/Logisim--Evolution-333333?logo=data:image/jpeg;base64,)](https://github.com/logisim-evolution/logisim-evolution)

## Core Projects
- [aarch64-emulator](https://github.com/aschombe/aarch64-emulator) - A modular AArch64 architecture emulator written in Rust, featuring a custom POSIX system call emulation layer, native ELF binary loading, an interactive TUI debugger, and an embedded Lua runtime interface for custom scripting extensions.
- AeroShield - A real-time UAV cyberattack detection and threat monitoring framework featuring an automated flight simulation pipeline, an interactive web UI dashboard for network/cryptographic alerting, and a complementary adversarial simulation engine built for tactical stress-testing.

## Connect with Me

[![Website](https://img.shields.io/badge/Website-aschombe.github.io-333333?logo=Firefox&logoColor=orange)](https://schombe.me)

[![GitHub](https://img.shields.io/badge/GitHub-aschombe-333333?logo=github)](https://github.com/aschombe)

[![LinkedIn](https://custom-icon-badges.demolab.com/badge/LinkedIn-andrew--schomber-333333?logo=linkedin-white)](https://www.linkedin.com/in/andrew-schomber/)

[![Email](https://img.shields.io/badge/Email-contact%40schombe.me-333333?style=flat&logo=minutemailer&logoColor=4A90E2)](mailto:contact@schombe.me)
